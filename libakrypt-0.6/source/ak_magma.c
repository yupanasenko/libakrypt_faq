...
/* ----------------------------------------------------------------------------------------------- */
 static inline ak_uint32 ak_magma_gostf( ak_uint32 x )
{
 /* очевидно, что можно хранить в памяти 32-х битные сдвинутые числа ! */

  x = ( (ak_uint32)( magma_boxes[3][(x>>24)&255] ) << 24 ) |
      ( (ak_uint32)( magma_boxes[2][(x>>16)& 255] ) << 16 ) |
      ( (ak_uint32)( magma_boxes[1][(x>>8)& 255] ) <<  8 ) | ( (ak_uint32)( magma_boxes[0][x&255] ));

 /* заменить на один вызов циклического сдвига */
  return x<<11 | x>>(32-11);
}
