...
/*  Minimal patching for accordance to libakrypt by Axel Kenzo */
#include <ak_oid.h>
...
OBJECT_IDENTIFIER_print(asn_TYPE_descriptor_t *td, const void *sptr,
    ...
    const OBJECT_IDENTIFIER_t *st = (const OBJECT_IDENTIFIER_t *)sptr;
    ak_oid oid = ak_oid_find_by_object_identifier( st );
    ...
    if( oid != NULL ) { /* значение найдено в списке библиотеки */
      cb( oid->name, strlen( oid->name ), app_key );
      cb( " { ", 3, app_key );
      cb( oid->id, strlen( oid->id ), app_key );
