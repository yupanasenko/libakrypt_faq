...
aktool - консольная утилита для шифрования, контроля целостности и вычисления электронной подписи.
...
*asn1parse*  Декодирование и печать ASN.1 данных
...
Короткая форма команды *asn1parse*.

## asn1parse [*опции*] *файл[ы]*

Команда позволяет декодировать и вывести на печать данные,
закодированные в виде ASN.1 структуры. В настоящее время поддерживается разбор
чистых der-последовательностей, а также der-последовательностей,
закодированных в кодировке base64 (формат сертификатов pem).

Доступны следующие опции:

\--no-check
:   Опция заставляет утилиту не выполнять обязательную проверку -
перед выводом данных в консоль проводится тестирование корректности декодирования.
Для этого данные повторно закодируются, после чего проводится сравнение
считанных из файла и закодированных утилитой aktool данных. Поскольку утилита использует внутренний буффер
фиксированного размера,
то на файлах большого размера проверка будет выдавать ошибочный результат.
...
Приведенный ниже вызов утилиты вычисляет контрольную сумму файла file.txt с помощью установленного по умолчанию
алгоритма "Стребог256" и помещает результат вычислений в файл result.txt.

    aktool i file.txt -o result.txt

Следующий вызов позволяет вычислить код целостности всех файлов, удовлетворяющих шаблону "*.t??"
...
Следующий вызов позволяет проверить контрольные суммы, указанные в файле result.txt.
...
Последний вызов позволяет проверить значения имитовставок, указанных в файле result.txt
