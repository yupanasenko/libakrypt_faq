...
Перечень доступных алгоритмов имитозащиты и вычисления контрольной суммы
...
der-последовательностей, закодированных в кодировке base64 (формат сертификатов pem).

\--convert
: Опция позволяет выполнить конвертацию ASN.1 данных между поддерживаемыми форматами,
т.е. перевести данные в der-формата в pem-формат и наоборот.
...
Опци может применяться для разделения коллекций сертификатов открытых ключей на
отдельные файлы.
...
При **\--to=der** значение опции игнорируется.
...
содержащится в качестве примера в рекомендациях Р 1323565.1.023-2018, раздел А.2.2.1.
...
В результате декодирования в консоль должна быть выведена следующая иерархическая структура.
    ...
    ┌SEQUENCE┐
         ...
         │        ├SEQUENCE┐
         │        │        ├SEQUENCE┐
         │        │        │        ├OBJECT IDENTIFIER 1.2.643.7.1.1.1.1 (id-tc26-gost3410-12-256)
         │        │        │        └SEQUENCE┐
         │        │        │                 └OBJECT IDENTIFIER 1.2.643.7.1.2.1.1.1 (id-tc26-gost-3410-2012-256-paramSetA)
         │        │        └BIT STRING
         ...
         │             (null)
         ├SEQUENCE┐
         │        └OBJECT IDENTIFIER 1.2.643.7.1.1.3.2 (id-tc26-signwithdigest-gost3410-12-256)
         └BIT STRING
...
Пример конвертирования ASN.1 дерева, сохраненного в виде der-последовательности, в формат pem.


    aktool a --convert request_edw.p10 --to pem --pem request -o request_edw.pem


Далее приводится пример того, как можно разделить коллекцию сертификатов на отдельные файлы.
...
или идентификатором. Перечень всех известных библиотеке идентификаторов эллиптических кривых
может быть получен с помощью следующего вызова.
...
Указанная опция запрещает использование цвета, что может быть полезно при просмотре вывода утилиты другими программами,
например, **less**.
