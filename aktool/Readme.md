...
меткой *used for testing purpose* и сохранен в файл `hmac.k`.
    ...
    aktool k -na hmac-streebog256 -o hmac.key --label "used for testing purpose"
...
Создание ключевой пары со случайными именами файлов и использованием 
символьного устройства `/dev/urandom` в качестве генератора случайных последовательностей.


    aktool k -na sign256 --random-file /dev/urandom


Создание ключевой пары для алгоритма электронной подписи ГОСТ Р 34.10-2012 
с длиной подписи 512 бит, с явным указанием эллиптической кривой,
которой принадлежит открытый ключ.

Секретный ключ помещается в файл `secret.key`, а открытый ключ сохраняется 
в форме *запроса на сертификат* в файле `public.csr`. Оба ключа сохраняются в формате `pem`.


    aktool k -na sign256 --curve cspa -o secret.key --op public.csr --to pem


Создание ключевой пары для электронной подписи ГОСТ Р 34.10-2012  с длиной подписи 1024 бита,
с явным указанием эллиптической кривой и с сохранением созданного открытого ключа
в форме *самоподписанного сертификата*.


    aktool k -na sign512 --curve id-tc26-gost-3410-2012-512-paramSetA \ 
           -o ca.key --op ca.crt --to certificate


Корректность выработанных выше контейнеров открытых ключей (запроса на сертификат и сертификата) 
также может быть проверена другими программами, например, `openssl`.


    openssl req -text -noout -verify -in public.csr
    openssl verify -CAfile public512 public512.crt
...
[убрать](https://blog.ukrnames.com/bezopasnost/openssl-proverka-ssl-sertifikatov-cherez-terminal).
