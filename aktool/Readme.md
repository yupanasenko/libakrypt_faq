...
Пример декодирования ASN.1 дерева, содержащего запрос на создание сертификата открытого ключа
в формате pem.
...
\--audit \<file\>
: Опция позволяет определить файл, в который будут помещаться сообщения о процессе работы программы и ошибках выполнения.
В случае, если значение аргумента опции принимает значение **stderr**,
то используется установленный по умолчанию канал вывода ошибок.
