...
## Изменения в версии 0.9.3
 ...
 - Структура секретного ключа дополнена полем label (метка ключа)
 - Оптимизирована реализация функций импорта ключевой информации,
   теперь можно считывать параметры ключа без считывания собственно ключевой информации
 - Сделан экспорт секретных ключей в файл в незашифрованном виде, 
   функция ak_skey_export_to_file_unencrypted()
 - Расширен функционал утилиты aktool, в частности, возвращено создание сертификатов 
   открытых ключей из запросов на сертификат 
