 ...
 int main( void )
{
 ...
 /* 1. создаем генератор псевдо-случайных чисел */
  if(( error = ak_random_create_lcg( generator = malloc( sizeof( struct random )))) != ak_error_ok ) {
    free( generator );
    return ak_libakrypt_destroy();
    /* фиксируем последовательность, вырабатываемую генератором */
  }

 /* 2. создаем пустой контекст ключа */
 ...
 /* 3. присваиваем ключу константное значение,
 ...
 /* выводим последовательность, которую должен выработать генератор */
 ...
 /* 4. не удаляя старое значение ключа, присваиваем новое, как бы случайное.
 ...
 /* 5. повторяем процедуру еще раз, теперь с ключом из пароля */
 ...
 /* 6. освобождаем память и выходим */
  exit:
