 ...
 int main( void )
{
 ...
 /* начнем с того, что проверим корректность работы для заданных констант */
  key = ak_bckey_new_magma_ptr( gost3412_2015_key, 32, ak_false );
  print_key( &key->key );
 ...
 /* теперь зашифруем те же блоки случайным ключом */
  key = ak_bckey_new_magma_ptr( gost3412_2015_key, 32, ak_false );
  print_key( &key->key );
  key->encrypt( &key->key, a, out );
  printf("out: %s\n", str = ak_ptr_to_hexstr( out, 8, ak_true )); free( str );
  key->decrypt( &key->key, out, out );
  printf("in:  %s\n\n", str = ak_ptr_to_hexstr( out, 8, ak_true )); free( str );
  key = ak_bckey_delete( key );

 /* в заключение зашифруем те же блоки на ключе, выработанном из пароля */
  key = ak_bckey_new_magma_password( "password", 8 );
  print_key( &key->key );
  key->encrypt( &key->key, a, out );
  printf("out: %s\n", str = ak_ptr_to_hexstr( out, 8, ak_true )); free( str );
  key->decrypt( &key->key, out, out );
