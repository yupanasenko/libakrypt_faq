...
7. Арифметика Монтгомери для реализации вычислений в конечных простых полях
...
платформах: x86, x64, armv7 (все little endian). Библиотека
...
Также были проведены успешние запуски библиотеки под управлением `ReactOS` и `SailfishOS`.
...
Библиотека использует функции, определяемые `POSIX Threads`.
