...
int asn_put_len(size_t len, byte* p_buff)
{
    ...
    int8_t len_byte_cnt = asn_get_len_byte_cnt(len);
...
int asn_put_objid(object_identifier obj_id, byte* p_buff)
{
    ...
    size_t num = (size_t)strtoul((char*)obj_id, &p_objid_end, 10);
    obj_id = ++p_objid_end;
...
int asn_put_generalized_time(generalized_time time, byte* p_buff)
{
    ...
    int8_t byte_cnt = asn_get_gentime_byte_cnt(time);
    if(byte_cnt < 15)
    ...
    if(*time == '.')
    {
        ...
        if(time[ms_cnt] == '0')
            return ak_error_message(ak_error_wrong_asn1_encode, __func__, "wrong format of quota of second value (it can't end by 0 symbol)");
