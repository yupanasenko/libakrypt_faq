...
/*! В режиме ACPKM для шифрования используется операция гаммирования - операция сложения открытого
    ...
    функции, не изменяется. Повторный вызов функции ak_bckey_context_ctr_acpkm() с нулевым указатетем
    на синхропосылу, как в случае функции ak_bckey_context_ctr(), не допускается.
