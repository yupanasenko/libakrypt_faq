/* ----------------------------------------------------------------------------------------------- */
/*  Copyright (c) 2008 - 2010, 2016 - 2018 by Axel Kenzo, axelkenzo@mail.ru                        */
/*                                                                                                 */
/*  Файл ak_hash.h                                                                                 */
...
/* ----------------------------------------------------------------------------------------------- */
 int ak_hash_context_create_gosthash94( ak_hash ctx, ak_oid oid )
{
 ...
 /* инициализируем контекст */
  if(( error = ak_hash_context_create( ctx, sizeof( struct gosthash94 ), 32 )) != ak_error_ok )
 ...
 /* устанавливаем OID алгоритма хеширования */
  if(( ctx->oid = ak_oid_context_find_by_name( "gosthash94" )) == NULL )
...
/*! Функция инициализирует контекст алгоритма бесключевого хеширования, регламентируемого стандартом
    ...
    @return Функция возвращает код ошибки или \ref ak_error_ok (в случае успеха)                   */
/* ----------------------------------------------------------------------------------------------- */
 int ak_hash_context_create_gosthash94_csp( ak_hash ctx )
{
  return ak_hash_context_create_gosthash94( ctx,
                                 ak_oid_context_find_by_name( "id-gosthash94-rfc4357-paramsetA" ));
}
 ...
 ak_bool ak_hash_test_gosthash94( void )
{
 ...
 /* первый пример из ГОСТ Р 34.11-94 */
  if(( error = ak_hash_context_create_gosthash94( &ctx,
                ak_oid_context_find_by_name( "id-gosthash94-test-paramset" ))) != ak_error_ok ) {
 ...
 /* первый пример для таблиц замен из Верба-О */
  if(( error = ak_hash_context_create_gosthash94( &ctx,
                ak_oid_context_find_by_name( "id-gosthash94-verbaO-paramset" ))) != ak_error_ok ) {
    ak_error_message( error, __func__ , "wrong initialization of gosthash94 context" );
    return ak_false;
  }
