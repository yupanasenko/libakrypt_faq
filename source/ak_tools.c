...
/*                                                                                                 */
/*  ak_tools.с                                                                                     */
/*  Файл содержит реализацию служебных функций, не экспортируемых за пределы библиотеки            */
/* ----------------------------------------------------------------------------------------------- */
 #include <ak_tools.h>
...
/* ----------------------------------------------------------------------------------------------- */
/*! @param hpath Буффер в который бут помещено имя домашнего каталога пользователя.
    @param size Размер буффера в байтах.

    @return В сулчае возникновения ошибки возвращается ее код. В случае успеха
...
/*! Функция проверяет наличие файла путем попытки открыть его на чтение. Если файл не существует,
    ...
    вызов функции равносилен вызову функции ak_file_open( ) с параметрами `O_RDONLY | O_BINARY`.
