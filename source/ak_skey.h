...
/* ----------------------------------------------------------------------------------------------- */
/*! \brief Перечисление определяет возможные типы ресурсов секретных ключей. */
 typedef enum {
...
/*! \brief Перечисление, определяющее флаги хранения и обработки секретных ключей. */
 typedef enum {
  ...
  /*! \brief Третий бит отвечает за то, кто из классов skey или его наследники уничтожают
             внутреннюю память. Если флаг установлен, то класс skey очистку не производит и
             возлагает это на методы классов-наследников:
             0 - очистка производится, 1 - очистка памяти не производится */
   skey_flag_data_not_free = 0x08LL,

 /*! \brief Флаг, который запрещает использование функции ctr без указания синхропосылки. */
   bckey_flag_not_ctr = 0x0100LL,
...
/* ----------------------------------------------------------------------------------------------- */
/*! \brief Инициализация структуры секретного ключа значением другого ключа */
...
/*! \brief Процедура вычисления производного ключа в соответствии с алгоритмом ACPKM
 ...
 int ak_skey_context_next_acpkm_key( ak_skey );
...
/* ----------------------------------------------------------------------------------------------- */
/*! \brief Функция присваивает ресурсу ключа значение, определенное заданной опцией библиотеки. */
 int ak_skey_context_set_resource( ak_skey , resource_t , const char * );
