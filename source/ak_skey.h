...
/*! \brief Указатель на структуру секретного ключа асимметричного криптографического алгоритма. */
 typedef struct ecskey *ak_ecskey;
/*! \brief Указатель на структуру открытого ключа асимметричного криптографического алгоритма. */
 typedef struct ecpubkey *ak_ecpubkey;
...
/*! \brief Наложение мультипликативной маски на ключ. */
 int ak_skey_set_mask_multiplicative( ak_skey );
...
/*! \brief Смена значения мультипликативной маски. */
 int ak_skey_remask_multiplicative( ak_skey );
/*! \brief Вычисление значения контрольной суммы ключа. */
 int ak_skey_set_icode_additive( ak_skey );
/*! \brief Вычисление значения контрольной суммы ключа. */
 int ak_skey_set_icode_xor( ak_skey );
/*! \brief Вычисление значения контрольной суммы ключа. */
 int ak_skey_set_icode_multiplicative( ak_skey );
...
/*! \brief Проверка значения контрольной суммы ключа. */
 ak_bool ak_skey_check_icode_multiplicative( ak_skey );
...
/*! \brief Функция выполняет тестирование алгоритма Магма. */
 ak_bool ak_bckey_test_magma( void );
/*! \brief Функция выполняет тестирование алгоритма Кузнечик. */
 ak_bool ak_bckey_test_kuznechik( void );
/*! \brief Инициализация статических таблиц, используемых в алгоритме Кузнечик. */
...
/* ----------------------------------------------------------------------------------------------- */
/*! \brief Секретный ключ асимметричных алгоритмов. */
/*! Секретный ключ представляет собой вектор длины 256 или 512 бит, интерпретируемый как
    экспонента фиксированной точки эллиптической кривой. Данный ключ однозначно связан
    с открытым ключом с помощью равенства \f$ Q = [d]P\f$, где
    - \f$ d \f$ секретный ключ асимметричного алгоритма,
    - \f$ P \f$ фиксированная точка на эллиптической кривой,
    - \f$ Q \f$ октрытый ключ асимметричного алгоритма.

    В качестве асимметричных алгоритмов могут выступать:
    - схема электронной подписи ГОСТ Р 34.10-2012,
    - гибридная схема шифрования,
    - протоколы выработки разового ключа на основе схемы Диффи-Хеллмана, реализуемой
...
/* ----------------------------------------------------------------------------------------------- */
/*! \brief Инициализация ключа алгоритма блочного шифрования. */
 int ak_ecskey_create( ak_ecskey , ak_wcurve_paramset );
/*! \brief Создание контекста ключа алгоритма блочного шифрования. */
 ak_bckey ak_ecskey_new( ak_wcurve_paramset );
/*! \brief Очистка ключа алгоритма блочного шифрования. */
 int ak_ecskey_destroy( ak_ecskey );
/*! \brief Удаление ключа алгоритма блочного шифрования. */
 ak_pointer ak_ecskey_delete( ak_pointer );


/* ----------------------------------------------------------------------------------------------- */
/*! \brief Открытый ключ асимметричных алгоритмов. */
/*! Открытый ключ представляет собой точку эллиптической кривой. Данный ключ однозначно связан
    с секретным ключом с помощью равенства \f$ Q = [d]P\f$, где
    - \f$ d \f$ секретный ключ асимметричного алгоритма,
    - \f$ P \f$ фиксированная точка на эллиптической кривой,
    - \f$ Q \f$ октрытый ключ асимметричного алгоритма.
