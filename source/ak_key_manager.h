...
/*! \brief Функция для поиска ключа.
    ...
    либо последовательность байт, входящую в номер ключа.

    В случае успешного поиска функция должна возвращать \ref ak_error_ok. Также должны
    инициализироваться значения типа криптографического преобразования (\ref oid_engines_t),
    для которого предназначен ключ, а также формат (\ref key_storage_farmat_t), в котором сохранен
    секретный ключ.

    В случае, если ключ с запрашиваемым номером не найден, должно возвращаться
    значение \ref ak_error_key_search.                                                             */
/* ----------------------------------------------------------------------------------------------- */
 typedef int ( ak_function_find_key_buffer )
                          ( ak_key_manager , ak_buffer , oid_engines_t *, key_storage_format_t * );

/* ----------------------------------------------------------------------------------------------- */
/*! \brief Функция для поиска ключа.
    \details Функция предназначена для поиска ключа с заданным номером. Для передачи значения номера
    ключа должна использоваться null-строка, содержащая либо номер ключа,
    либо последовательность байт, входящую в номер ключа.
    ...
    значение \ref ak_error_key_search.                                                             */
/* ----------------------------------------------------------------------------------------------- */
 typedef int ( ak_function_find_key_char )
                             ( ak_key_manager , char * , oid_engines_t *, key_storage_format_t * );
...
/* ----------------------------------------------------------------------------------------------- */
 typedef ak_pointer ( ak_function_get_key_buffer )
                 ( ak_key_manager , ak_buffer , oid_engines_t , key_storage_flags_t , ak_pointer );


/* ----------------------------------------------------------------------------------------------- */
/*! \brief Функция получения ключа по его номеру.
    \details По заданному номеру ключа, передаваемому в виде null-строки,
    функция должна возвращать указатель на созданный контекст ключа, размещенный в оперативной памяти.
    ...
    Узнать тип хранящегося ключа по его номеру, а также формат хранения можно с помощью
    функции типа \ref ak_function_find_key_char.
    ...
    можно получить с помощью функции ak_error_get_value(). */
/* ----------------------------------------------------------------------------------------------- */
 typedef ak_pointer ( ak_function_get_key_char )
                    ( ak_key_manager , char * , oid_engines_t , key_storage_flags_t , ak_pointer );
...
/* ----------------------------------------------------------------------------------------------- */
 struct key_manager {
  ...
  /*! \brief Функция поиска ключа. */
   ak_function_find_key_buffer *find;
  /*! \brief Функция поиска ключа. */
   ak_function_find_key_char *find_char;
  /*! \brief Функция получения ключа из ключевого хранилища. */
   ak_function_get_key_buffer *get;
  /*! \brief Функция получения ключа из ключевого хранилища. */
   ak_function_get_key_char *get_char;
};
