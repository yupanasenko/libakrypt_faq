...
/* ----------------------------------------------------------------------------------------------- */
/*! \brief Класс, уточняющий строение области статической памяти менеджера ключей.
    \details Данный менеджер ключей реализует хранение ключевой информации в заданном каталоге.
    Каждый ключ хранится независимо в отдельном файле.                                             */
/* ----------------------------------------------------------------------------------------------- */
                                            ...
                                            const char *container , crypto_content_t content_type )
{
 ...
 /* изменяем индексный файл */
  ...
  if(( error = ak_file_open_to_append( &f, filename )) != ak_error_ok ) {
    ak_error_message_fmt( error, __func__, "the index file %s cannot be changed", filename );
    goto lab1;
  }

 /* записываем информацию в текстовом виде */
