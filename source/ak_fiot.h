...
/* ----------------------------------------------------------------------------------------------- */
/*! \brief  Тип функции низкого уровня для чтения/записи из сокета. */
 typedef ssize_t ( fiot_function_write_socket )( ak_socket , const void *, size_t );
/*! \brief  Тип функции низкого уровня для чтения/записи из сокета. */
 typedef ssize_t ( fiot_function_read_socket )( ak_socket , void *, size_t );
...
*/
 typedef struct fiot {
  ...
  /*! \brief Значение счетчика, определяющего количество фреймов, зашифрованных на одном ключе. */
   size_t lcounter;
  /*! \brief Значение счетчика, определяющего количество преобразований ключей шифрования и имитозащиты. */
   size_t mcounter;
  /*! \brief Значение счетчика, определяющего количество преобразований общей ключевой информации. */
   size_t ncounter;
  ...
  /*! \brief Указатель на функцию записи данных в канал связи. */
   fiot_function_write_socket *write;
  /*! \brief Указатель на функцию получения данных из канала связи. */
   fiot_function_read_socket *read;
...
/*! \brief Вывод содержимого фрейма с использованием системы аудита. */
 int ak_fiot_context_log_frame( ak_uint8 *, size_t );
