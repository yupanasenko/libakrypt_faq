...
int pkcs_15_put_kekri(s_der_buffer* p_pkcs_15_token, s_kekri* p_kekri, s_der_buffer* p_kekri_der)
{
    ...
    if ((error =
                 ps_set(p_kekri_der, p_pkcs_15_token->mp_curr, kekri_len+asn_get_len_byte_cnt(kekri_len)+1, PS_U_MODE))
            !=ak_error_ok)
        return ak_error_message(error, __func__, "problems with making union of asn data");
        ...
        s_der_buffer* p_encrypted_content_info_der)
{
    ...
    if ((error =
                 ps_move_cursor(p_pkcs_15_token, encrypted_contetnt_len+asn_get_len_byte_cnt(encrypted_contetnt_len)+1))
            !=ak_error_ok)
        return ak_error_message(error, __func__, "problems with moving cursor");
...
int pkcs_15_get_encrypted_content_info(s_der_buffer* p_enveloped_data_der, s_enveloped_data* p_enveloped_data)
{
    ...
    if ((error = asn_get_expected_tlv(TOBJECT_IDENTIFIER, &enc_content_info, (void*) &p_enveloped_data->m_content_type))
            !=ak_error_ok)
...
void pkcs_15_free_enveloped_data(s_enveloped_data* p_ed)
{
    ...
    if (p_ed)
    {
        ...
        {
        case GOST_CONTENT_ENC_SET:
            asn_free_objid(&p_ed->m_prm_set.p_content_enc_prm_set->m_encryption_param_set);
...
void pkcs_15_free_kekri(s_kekri* p_kekri)
{
    if (p_kekri)
    {
        ...
        {
        case GOST_KEY_WRAP_SET:
            asn_free_octetstr(&p_kekri->m_prm_set.p_key_wrap_set->m_ukm);
