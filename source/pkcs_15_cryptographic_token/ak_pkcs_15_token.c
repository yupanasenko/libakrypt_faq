        ...
        s_der_buffer* p_added_pkcs_15_object_der)
{
    ...
    {
    case SEC_KEY:asn_put_tag(CONTEXT_SPECIFIC | CONSTRUCTED | 3u, p_pkcs_15_token->mp_curr);
        break;
    case PRI_KEY:return ak_error_message(ak_error_invalid_value, __func__, "adding private keys does not realized yet");
    case PUB_KEY:return ak_error_message(ak_error_invalid_value, __func__, "adding public keys does not realized yet");
...
int pkcs_15_get_obj(s_der_buffer* p_pkcs_15_objects_der, s_pkcs_15_object* p_pkcs_15_object)
{
    ...
    switch (tag)
    {
    case (CONTEXT_SPECIFIC | CONSTRUCTED | SEC_KEY):p_pkcs_15_object->m_type = SEC_KEY;
        break;
...
int pkcs_15_get_direct_obj(s_der_buffer* p_pkcs_15_object_der, s_pkcs_15_object* p_pkcs_15_object)
{
    ...
    switch (tag)
    {
    case (CONTEXT_SPECIFIC | CONSTRUCTED | 27u):p_pkcs_15_object->m_obj.mp_sec_key = calloc(1, sizeof(s_gost_sec_key));
...
void free_key_management_info(s_key_management_info* p_kmi)
{
    if (p_kmi)
    {
        switch (p_kmi->m_type)
        {
        case PWD_INFO:free_pwd_info(p_kmi->m_key_info.mp_pwd_info);
...
void free_pkcs_15_object(s_pkcs_15_object* p_object)
{
    if (p_object)
    {
        ...
        {
        case SEC_KEY: pcks_15_free_gost_sec_key(p_object->m_obj.mp_sec_key);
            break;
        case PRI_KEY: break;
        case PUB_KEY: break;
        default: break;
