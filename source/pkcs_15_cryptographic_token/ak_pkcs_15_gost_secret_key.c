...
int pkcs_15_put_gost_key(s_der_buffer *p_pkcs_15_token_der, s_gost_sec_key *p_key, s_der_buffer *p_gost_key_der)
{
    ...
    0 == strcmp(p_key->m_key_type_gost, "1.2.643.7.1.1.5.2"))
    {
        ...
        return ak_error_message_fmt(ak_error_wrong_oid, __func__, "this type of key (%s) is prohibited. Allowed only other (%s)", p_key->m_key_type_gost, "1.2.643.2.2.21");
    ...
    if ((error = ps_set(p_gost_key_der, p_pkcs_15_token_der->mp_curr, gost_key_der_len + asn_get_len_byte_cnt(gost_key_der_len) + 1, PS_U_MODE)) != ak_error_ok)
...
int pkcs_15_put_key_attr(s_der_buffer *p_pkcs_15_token_der, s_gost_sec_key *p_key, s_der_buffer *p_key_attr_der)
{
    ...
    if((error = pkcs_15_put_gost_secret_key_attributes(p_pkcs_15_token_der, p_key, &gost_secret_key_attributes_der)) != ak_error_ok)
    ...
    {
        if((error = pkcs_15_put_common_secret_key_attributes(p_pkcs_15_token_der, p_key, &common_secret_key_attributes_der)) != ak_error_ok)
            return ak_error_message(error, __func__, "problem with adding common secret key attributes");
    }

    /* Добавляем общие атрибуты ключей  */
    if((error = pkcs_15_put_common_key_attributes(p_pkcs_15_token_der, &p_key->m_key_attrs, &common_key_attributes_der)) != ak_error_ok)
        return ak_error_message(error, __func__, "problem with adding common key attributes");

    /* Добавляем общие атрибуты объекта PKCS 15 */
    if((error = pkcs_15_put_common_object_attributes(p_pkcs_15_token_der, &p_key->m_obj_attrs, &common_object_attributes_der)) != ak_error_ok)
    ...
    if((error = asn_put_universal_tlv(TSEQUENCE, NULL, key_attr_der_len, p_pkcs_15_token_der, p_key_attr_der)) != ak_error_ok)
...
//  if(p_key->m_key_info.mp_parameters)
//  {
...
//  }
    ...
    if((error = pkcs_15_put_object_direct_protected(p_pkcs_15_token_der, &p_key->m_enveloped_data, &direct_protected_key_der)) != ak_error_ok)
    ...
    if ((error = ps_move_cursor(p_pkcs_15_token_der, asn_get_len_byte_cnt(gost_sec_key_attrs_der_len) + 1)) != ak_error_ok)
        return ak_error_message(error, __func__, "problems with moving cursor");
    ...
    if ((error = ps_set(p_gost_secret_key_attributes_der, p_pkcs_15_token_der->mp_curr,
                        gost_sec_key_attrs_der_len + asn_get_len_byte_cnt(gost_sec_key_attrs_der_len) + 1, PS_U_MODE)) != ak_error_ok)
        return ak_error_message(error, __func__, "problems with making union of asn data");
                                             ...
                                             s_der_buffer *p_common_secret_key_attributes_der)
{
    ...
    if((error = asn_put_universal_tlv(TINTEGER, (void*)&p_key->m_key_len, 0, p_pkcs_15_token_der, &common_sec_key_attrs_der)) != ak_error_ok)
    ...
    if ((error = ps_move_cursor(p_pkcs_15_token_der, asn_get_len_byte_cnt(common_sec_key_attrs_der_len) + 1)) != ak_error_ok)
        return ak_error_message(error, __func__, "problems with moving cursor");
    ...
    if ((error = ps_set(p_common_secret_key_attributes_der, p_pkcs_15_token_der->mp_curr,
                        common_sec_key_attrs_der_len + asn_get_len_byte_cnt(common_sec_key_attrs_der_len) + 1, PS_U_MODE)) != ak_error_ok)
        return ak_error_message(error, __func__, "problems with making union of asn data");
...
int pkcs_15_put_key_info(s_der_buffer *p_pkcs_15_token_der, s_key_info *p_key_info, s_der_buffer *p_key_info_der)
{
    ...
    {
        if((error = asn_put_universal_tlv(TBIT_STRING, (void*)&p_key_info->m_supported_operations, 0, p_pkcs_15_token_der, &supported_operations_der)) != ak_error_ok)
            return ak_error_message(error, __func__, "problem with adding gost supported operations");
    }

    /* Добавляем идентификатор параметров */
    if(p_key_info->mp_parameters->m_id != NULL)
    {
        s_der_buffer crypto_pro_param_set = {0};
        if (0 == strcmp(p_key_info->mp_parameters->m_id, "1.2.643.7.1.2.5.1.1"))
        {
            if((error = asn_put_universal_tlv(TOBJECT_IDENTIFIER, (void*)&p_key_info->mp_parameters->m_id, 0, p_pkcs_15_token_der, &crypto_pro_param_set)) != ak_error_ok)
            ...
            return ak_error_message_fmt(ak_error_wrong_oid, __func__, "algorithm (%s) doesn't support", p_key_info->mp_parameters->m_id);
    }
    ...
    if((error = asn_put_universal_tlv(TSEQUENCE, NULL, prms_and_ops_der_len, p_pkcs_15_token_der, &prms_and_ops_der)) != ak_error_ok)
        return ak_error_message(error, __func__, "problem with adding sequence tag and length");
...
{
    if(!p_pkcs_15_token_der || !p_params || !p_params_der || !p_params->m_iv.mp_value || !p_params->m_encryption_param_set)
        return ak_error_message(ak_error_null_pointer, __func__, "invalid arguments");
    ...
    if((error = asn_put_universal_tlv(TOBJECT_IDENTIFIER, (void *) &p_params->m_encryption_param_set, 0, p_pkcs_15_token_der, &encryption_param_set)) != ak_error_ok)
        return ak_error_message(error, __func__, "problem with adding content encryption parameter set id");

    s_der_buffer iv = {0};
    if((error = asn_put_universal_tlv(TOCTET_STRING, (void *) &p_params->m_iv, 0, p_pkcs_15_token_der, &iv)) != ak_error_ok)
...
int pkcs_15_get_gost_key(s_der_buffer *p_object_der, s_gost_sec_key *p_key)
{
    ...
    if((error = asn_get_expected_tlv(CONTEXT_SPECIFIC | CONSTRUCTED | 27u, p_object_der, (void *) &gost_key_der)) != ak_error_ok)
    ...
    if((error = asn_get_expected_tlv(TOBJECT_IDENTIFIER, &gost_key_der, (void *) &p_key->m_key_type_gost)) != ak_error_ok)
...
int pkcs_15_get_common_secret_key_attributes(s_der_buffer *p_key_der, s_gost_sec_key *p_key)
{
    ...
    if((error = asn_get_expected_tlv((CONTEXT_SPECIFIC | CONSTRUCTED | 0u), p_key_der, (void *) &secret_key_attrs )) != ak_error_ok)
...
int pkcs_15_get_gost_secret_key_attributes(s_der_buffer *p_key_der, s_gost_sec_key *p_key)
{
    ...
    if((error = asn_get_expected_tlv((CONTEXT_SPECIFIC | CONSTRUCTED | 1u), p_key_der, (void *) &gost_key_attrs)) != ak_error_ok)
        return ak_error_message(error, __func__, "problems with getting common secret key attributes in der");
...
int pkcs_15_get_key_info(s_der_buffer *p_key_der, s_key_info* p_key_info)
{
    ...
    switch (curr_tag)
    {
        case TOBJECT_IDENTIFIER:
            p_key_info->mp_parameters = malloc(sizeof(s_algorithm_identifier));
            if((error = asn_get_expected_tlv(TOBJECT_IDENTIFIER, &key_info_der, (void *) &p_key_info->mp_parameters->m_id)) != ak_error_ok)
    ...
    {
        if((error = asn_get_expected_tlv(TBIT_STRING, &key_info_der, (void *) &p_key_info->m_supported_operations)) != ak_error_ok)
...
int pkcs_15_make_enc_key_plus_mac_seq(ak_buffer encrypted_cek, ak_buffer mac, ak_buffer encrypted_key_der)
{
    ...
    memcpy(enc_key_der.mp_curr + 1 + asn_get_len_byte_cnt(encrypted_cek->size), encrypted_cek->data, encrypted_cek->size);
