...
* `ak_foo_create()` - функция инициализации объекта класса `foo`; дання функция не должна вызываться напрямую,
...
если класс `foo` позволяет реализовывать два конкретных алгоритма, например, `mouse` и `penguin`,
