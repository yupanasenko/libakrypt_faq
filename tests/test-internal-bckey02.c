 ...
 int main( void )
{
  ...
  ak_uint8 outin[32] = {
    0x29, 0x54, 0x29, 0x5E, 0x8D, 0x71, 0x3A, 0x5B, 0x90, 0xB0, 0x33, 0x39, 0x9E, 0xBD, 0x3B, 0x24,
    0xC4, 0x63, 0xD8, 0x93, 0x6C, 0xB0, 0x27, 0x7F, 0x7B, 0xBC, 0xBF, 0xCA, 0xF9, 0x6F, 0x62, 0xE1 };
 ...
 /* 2. создаем ключ алгоритма блочного шифрования */
  if( ak_bckey_context_create_kuznechik( &key) != ak_error_ok ) {
 ...
 /* 4. зашифровываем данные */
   if( ak_bckey_context_encrypt_ecb( &key, in, out, sizeof( in )) == ak_error_ok )
     printf("encryption of two blocks is Ok\n");
    else printf("encryption of two blocks is wrong\n");
 ...
 /* 4. расшифровываем данные */
   if( ak_bckey_context_decrypt_ecb( &key, outin, out, sizeof( outin )) == ak_error_ok )
     printf("decryption of two blocks is Ok\n");
    else printf("decryption of two blocks is wrong\n");
