...
//теперь нужен тестовый пример для openssl
 ...
 /* устанавливаем секретный ключ */
  ak_bckey_context_set_key( &bkey, gost3412_2015_key, 32 );

 /* зашифровываем и расшифровываем всего блок данных в режиме простой замены */
  ak_bckey_context_encrypt_ecb( &bkey, a, buf, 8 );
  printf("encrypted: %s\n", ak_ptr_to_hexstr( buf, 8, ak_false ));
  printf(" expected: %s\n", ak_ptr_to_hexstr( b, 8, ak_false ));
