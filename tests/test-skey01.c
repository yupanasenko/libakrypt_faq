 ...
 void print_skey_info( ak_skey key )
{
 ...
 /* информация о ключе */
  ...
  printf("\n icode:\t");
  for( i = 0; i < sizeof( key->icode ); i++ ) printf("%02X", key->icode[i] );
...
////  ak_skey_context_set_key( &key, testkey, sizeof( testkey ), ak_true );
//// /* устанавливаем ресурс ключа */
////  ak_skey_context_set_resource( &key, key_using_resource, "hmac_key_count_resource", time(NULL), time(NULL)+2592000 );
